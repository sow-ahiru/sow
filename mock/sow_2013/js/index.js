...
$(window).load(function() {
	...
	$('#container').fullContent({
		stages: 'div',
		mapPosition: [{v: 1, h: 1}, {v: 2, h: 1}, {v: 3, h: 1}, {v: 4, h: 1}, {v: 5, h: 1}],
		stageStart: 1,
		speedTransition: 300,
		idComplement: 'p_'
	});
	...
	var subtract_offset_top = parseInt($('#p_gallery').css('top').replace(/px/g, ''));
			...
			var kinetic_moving				= false,
				...
				openItem					= function( $item ) {
					...
					// if image item
						...
						if ($item.children('img').data('largesrc') == '') {
							return;
						}
				...
				loadImgPreview				= function( $item, callback ) {
					...
					preloadImage( largeSrc, function() {
						...
						$('#ib-img-preview').css({
							...
							top		: $item.offset().top - subtract_offset_top
