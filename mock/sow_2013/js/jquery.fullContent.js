...
;(function ( $, window, undefined ) {
	...
	Plugin.prototype.configStage = function () {
		...
		if (window.location.hash) {
			...
			changeNavi('#'+this.options.idComplement+hash);
	...
	Plugin.prototype.bind = function () {
		...
		this.$window.bind( 'hashchange', function( event ){
			...
			changeNavi('#'+self.options.idComplement+hash);
	...
	function changeNavi(target) {
		...
		$('#navi').removeClass('bgBlack bgWhite');
		$('#navi .lHalf').removeClass('bgBlack bgWhite');

		$('#navi h2').removeClass('title01 title02');

		if ($(target).attr('navi') == 1) {
			$('#navi').addClass('bgBlack');
			$('#navi .lHalf').addClass('bgWhite');
			$('#navi h2').addClass('title01');

		} else {
			$('#navi').addClass('bgWhite');
			$('#navi .lHalf').addClass('bgBlack');
			$('#navi h2').addClass('title02');
		}

		$('.article:not(' + target + ' .article)').hide();
	}
